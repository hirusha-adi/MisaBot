...
class ModerationCommands(commands.Cog):
    ...
    @commands.command()
    async def slowmode(self, ctx, *, seconds: int=3):
        loading_message = await ctx.send(embed=self.please_wait_emb)
    ...
    async def newemoji(self, ctx, name, link, filetyple="png"):
        loading_message = await ctx.send(embed=self.please_wait_emb)
        try:
            try:
                image = reqget(link)
    ...
    @commands.has_permissions(manage_messages=True)
    @commands.command()
    async def slap(ctx, user: discord.Member, *, reason='Reason is not given'):
    ...
    @commands.has_permissions(manage_messages=True)
    @commands.command()
    async def tempmute(self, ctx, member: discord.Member, time: int = 3, d = "m", *, reason="No reason is provided"):
        loading_message = await ctx.send(embed=self.please_wait_emb)
    ...
    async def changeprefix(self, ctx, *, newprefix="<"):
        loading_message = await ctx.send(embed=self.please_wait_emb)
        await loading_message.delete()
        await ctx.send(f'This feature will be available in the future! Make sure to type the status command to see more information')
